...
### Added
- `factorforge_cds_compare` — compare multiple CDS design profiles side-by-side via `POST /api/optimize/compare`
- `factorforge_cds_batch` — process up to 20 sequences in a single request via `POST /api/optimize/batch`
- Experimental profile enum added for FactorForge v3.1.6, later removed from the public API surface.
...
- `factorforge_cds_optimize` — CDS candidate generation via FactorForge CDS
