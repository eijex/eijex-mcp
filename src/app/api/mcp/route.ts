...
const TOOLS = [
  {
    ...
    description: 'Generate an in-silico synonymous DNA coding sequence (CDS) candidate from a protein sequence using FactorForge CDS v3.2.7. Supports N. benthamiana (default) and Tobacco BY-2 (--host by2). Returns pre-synthesis sequence-review metrics and rule-scan output; wet-lab validation is required and no expression, yield, synthesis-acceptance, folding, glycosylation, or regulatory outcome is guaranteed.',
    inputSchema: {
      type: 'object',
      properties: {
        sequence: {
  ...
  },
  {
    ...
    description: 'Generate in-silico CDS candidates for multiple protein sequences in a single request using FactorForge CDS. Accepts up to 20 sequences. Returns CAI, GC%, designed CDS, and sequence-review metrics for each.',
    inputSchema: {
      type: 'object',
      properties: {
        sequences: {
